...
type Args struct {
	Entity string         `json:"entity" jsonschema:"containsprofanity | json | plain | xml"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.ProfanityFilterSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "profanity-filter_list",
		Description: "List records from ProfanityFilter. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "profanity-filter_load",
		Description: "Load a single record from ProfanityFilter. " +
...
func runOp(client *sdk.ProfanityFilterSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.ProfanityFilterSDK, name string) (sdk.ProfanityFilterEntity, error) {
	switch strings.ToLower(name) {
	case "containsprofanity":
		return client.Containsprofanity(nil), nil
	case "json":
		return client.Json(nil), nil
	case "plain":
		return client.Plain(nil), nil
	case "xml":
		return client.Xml(nil), nil
