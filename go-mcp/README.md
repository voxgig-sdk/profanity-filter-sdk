# profanity-filter-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ProfanityFilter SDK as
two agent tools — `profanity-filter_list` and `profanity-filter_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/profanity-filter-mcp)
make build

# 2. Provide credentials via the environment
export PROFANITY_FILTER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user profanity-filter \
  -- /absolute/path/to/profanity-filter-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./profanity-filter-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// profanity-filter_list: first page of records
{ "entity": "containsprofanity" }
{ "entity": "containsprofanity", "query": { } }

// profanity-filter_load: one record by id
{ "entity": "containsprofanity", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/profanity-filter-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user profanity-filter \
     -- "$PWD"/dist/*/profanity-filter-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `profanity-filter_list` and `profanity-filter_load` tools now appear
   in new sessions. Ask the agent to *"list containsprofanity using profanity-filter"*
   and it calls `profanity-filter_list` with `{"entity":"containsprofanity"}`.
...
```sh
export PROFANITY_FILTER_APIKEY=sk_live_xxx            # API key
export PROFANITY_FILTER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `profanity-filter_list` tool
...
### Call the `profanity-filter_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `profanity-filter_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `profanity-filter_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PROFANITY_FILTER_APIKEY` | API key sent with every request. |
| `PROFANITY_FILTER_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

containsprofanity | json | plain | xml

### Smoke test via HTTP (raw JSON-RPC)

```sh
./profanity-filter-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"profanity-filter_load","arguments":{"entity":"containsprofanity","query":{"id":1}}}}'
```
