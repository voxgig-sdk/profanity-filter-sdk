...
class ProfanityFilterSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ProfanityFilterHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ProfanityFilterHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ProfanityFilterFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ProfanityFilterUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = ProfanityFilterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ProfanityFilterHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ProfanityFilterHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ProfanityFilterError.new(
        "graphql_error", "ProfanityFilterSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ProfanityFilterSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
