...
module ProfanityFilterUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ProfanityFilterContext.new({}, nil)
    end
    op = ctx.op || ProfanityFilterOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ProfanityFilterResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ProfanityFilterError) ? err.msg : err.to_s
    ...
    sdk_err = ProfanityFilterError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ProfanityFilterError)
