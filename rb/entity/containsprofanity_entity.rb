...
class ContainsprofanityEntity
  ...
  def make
    opts = @_entopts.dup
    ContainsprofanityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ProfanityFilterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Containsprofanity, Hash] the current Containsprofanity data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ProfanityFilterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Containsprofanity fields)
  def match_get
  ...
  #
  # @param reqmatch [ContainsprofanityLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Containsprofanity.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Containsprofanity, Hash] the loaded Containsprofanity; raises ProfanityFilterError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ProfanityFilterHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
