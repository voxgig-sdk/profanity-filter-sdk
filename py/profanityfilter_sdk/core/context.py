...
from profanityfilter_sdk.core.helpers import get_ctx_prop, to_map
...
class ProfanityFilterContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = ProfanityFilterControl()
        ...
        if isinstance(sp, ProfanityFilterSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ProfanityFilterResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ProfanityFilterResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return ProfanityFilterOperation({})
    ...
    def make_error(self, code, msg):
        return ProfanityFilterError(code, msg, self)
