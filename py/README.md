# ProfanityFilter Python SDK



The Python SDK for the ProfanityFilter API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Containsprofanity()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/profanity-filter-sdk/releases)) or
...
```python
client = ProfanityFilterSDK.test(testopts, sdkopts)
```
...
```
py/
├── profanityfilter_sdk.py         -- Main SDK module
...
The main module (`profanityfilter_sdk`) exports the SDK class.
