...
The Python SDK for the ProfanityFilter API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Containsprofanity(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.ProfanityFilter(None).load(
    {"id": "test01"}, None
)
