# ProfanityFilter SDK test runner

from __future__ import annotations
import os
import json

from profanityfilter_sdk.utility.voxgig_struct import voxgig_struct as vs


class ProfanityFilterTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ProfanityFilterTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ProfanityFilterTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ProfanityFilterTestRunner.getenv("PROFANITY_FILTER_TEST_LIVE")
        override = ProfanityFilterTestRunner.getenv("PROFANITY_FILTER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ProfanityFilterTestRunner.getenv(key)
        ...
        explain = ProfanityFilterTestRunner.getenv("PROFANITY_FILTER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ProfanityFilterTestRunner._test_control is not None:
            return ProfanityFilterTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ProfanityFilterTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ProfanityFilterTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ProfanityFilterTestRunner.load_test_control()
...
def load_env_local():
    ProfanityFilterTestRunner.load_env_local()


def env_override(m):
    return ProfanityFilterTestRunner.env_override(m)


def entity_data(v):
    return ProfanityFilterTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ProfanityFilterTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ProfanityFilterTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ProfanityFilterTestRunner.load_test_control()


def live_client_options():
    return ProfanityFilterTestRunner.live_client_options()


def live_delay_ms():
    return ProfanityFilterTestRunner.live_delay_ms()
