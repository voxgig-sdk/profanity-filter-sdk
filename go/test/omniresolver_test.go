...
import (
	...
	omni "github.com/voxgig-sdk/profanity-filter-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ProfanityFilterError); is {
				out := map[string]any{
					"name":    "ProfanityFilterError",
...
func omniCtx(arg any, client *sdk.ProfanityFilterSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ProfanityFilterSDK); is {
			client = live
