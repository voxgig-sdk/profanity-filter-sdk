...
func TestPlainEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		plainRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.plain")))
...
func plainBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PROFANITY_FILTER_TEST_PLAIN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PROFANITY_FILTER_TEST_PLAIN_ENTID"])
