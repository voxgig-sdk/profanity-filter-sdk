...
func TestJsonEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set PROFANITY_FILTER_TEST_JSON_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func jsonBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PROFANITY_FILTER_TEST_JSON_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PROFANITY_FILTER_TEST_JSON_ENTID"])
