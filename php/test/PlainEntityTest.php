...
class PlainEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = ProfanityFilterSDK::test(null, null);
        $ent = $testsdk->Plain(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = plain_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "plain." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set PROFANITYFILTER_TEST_PLAIN_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $plain_ref01_data = null;
        if (count($plain_ref01_data_raw) > 0) {
...
function plain_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/plain/PlainTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["plain01", "plain02", "plain03"] as $k) {
    ...
    $entid_env_raw = getenv("PROFANITYFILTER_TEST_PLAIN_ENTID");
