...
class ProfanityFilterMakeRequest
{
    public static function call(ProfanityFilterContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new ProfanityFilterResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new ProfanityFilterResponse($fetched);
        } else {
