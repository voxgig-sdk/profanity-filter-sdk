# ProfanityFilter PHP SDK Reference

Complete API reference for the ProfanityFilter PHP SDK.
...
#### `ProfanityFilterSDK::test($testopts = null, $sdkopts = null)`
...
#### `Containsprofanity($data = null)`

Create a new `ContainsprofanityEntity` instance. Pass `null` for no initial data.
...
Create a new `PlainEntity` instance. Pass `null` for no initial data.
