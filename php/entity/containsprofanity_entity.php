...
// ProfanityFilter SDK Containsprofanity entity
...
class ContainsprofanityEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ContainsprofanityEntity($this->_client, $opts);
    }

    /**
     * @param Containsprofanity|array $args Containsprofanity data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ProfanityFilterHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Containsprofanity|array The current Containsprofanity data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Containsprofanity fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ProfanityFilterHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Containsprofanity fields).
     */
    ...
    /**
     * Load a single Containsprofanity.
     *
     * @param ContainsprofanityLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ContainsprofanityLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Containsprofanity|array The loaded Containsprofanity as an assoc-array at the
     *   SDK boundary; throws ProfanityFilterError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ProfanityFilterHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
