...
// ProfanityFilter SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ProfanityFilterTestFeature extends ProfanityFilterBaseFeature
{
    ...
    public function init(ProfanityFilterContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ProfanityFilterContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ProfanityFilterParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ProfanityFilterContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ProfanityFilterContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ProfanityFilterParam::call($ctx, $k);
