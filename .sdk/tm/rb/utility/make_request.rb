# ProfanityFilter SDK utility: make_request
...
module ProfanityFilterUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ProfanityFilterResponse.new({})
    ...
    if fetch_err
      ...
      response = ProfanityFilterResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ProfanityFilterResponse.new(fetched)
    else
