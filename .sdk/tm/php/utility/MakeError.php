...
class ProfanityFilterMakeError
{
    public static function call(?ProfanityFilterContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ProfanityFilterContext([], null);
        }
        $op = $ctx->op ?? new ProfanityFilterOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ProfanityFilterResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ProfanityFilterError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ProfanityFilterError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof ProfanityFilterError) {
