...
// ProfanityFilter SDK retry feature
...
class ProfanityFilterRetryFeature extends ProfanityFilterBaseFeature
{
    ...
    public function init(ProfanityFilterContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ProfanityFilterContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ProfanityFilterContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ProfanityFilterContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
