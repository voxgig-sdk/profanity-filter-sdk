...
// ProfanityFilter SDK pipeline test
//
...
{
    private static function utility(): ProfanityFilterUtility
    {
        return new ProfanityFilterUtility();
    }
    ...
    private static function ctx(array $over = []): ProfanityFilterContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ProfanityFilterContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ProfanityFilterOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ProfanityFilterError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ProfanityFilterOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ProfanityFilterMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ProfanityFilterOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ProfanityFilterMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ProfanityFilterOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ProfanityFilterMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ProfanityFilterSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ProfanityFilterMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new ProfanityFilterResult([]),
        ]);
        [, $err] = ProfanityFilterMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ProfanityFilterSpec([]),
            'response' => null,
            'result' => new ProfanityFilterResult([]),
        ]);
        [, $err] = ProfanityFilterMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ProfanityFilterSpec(['step' => 's']),
            'response' => new ProfanityFilterResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ProfanityFilterResult([]),
        ]);
        [, $err] = ProfanityFilterMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ProfanityFilterSpec(['step' => 's']),
            'response' => new ProfanityFilterResponse(self::resp(200, ['v' => 1])),
            'result' => new ProfanityFilterResult([]),
        ]);
        [, $err] = ProfanityFilterMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ProfanityFilterSpec(['step' => 's']),
            'response' => new ProfanityFilterResponse(self::resp(200, ['v' => 2])),
            'result' => new ProfanityFilterResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ProfanityFilterMakeResponse::call($ctx);
    ...
    {
        $preset = new ProfanityFilterResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ProfanityFilterSpec([]),
            'response' => new ProfanityFilterResponse([]),
            'result' => new ProfanityFilterResult([]),
        ...
        [$got, $err] = ProfanityFilterMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ProfanityFilterResult([])]);
        [, $err] = ProfanityFilterMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ProfanityFilterSpec([]), 'result' => null]);
        [, $err] = ProfanityFilterMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ProfanityFilterSpec(['step' => 's']),
            'result' => new ProfanityFilterResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ProfanityFilterOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ProfanityFilterMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ProfanityFilterSpec(['step' => 's']),
            'result' => new ProfanityFilterResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ProfanityFilterOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ProfanityFilterMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ProfanityFilterResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ProfanityFilterSpec([]),
            'result' => new ProfanityFilterResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ProfanityFilterMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ProfanityFilterMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ProfanityFilterError('boom', 'boom');
        $utility->fetcher = function (ProfanityFilterContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ProfanityFilterSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ProfanityFilterMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ProfanityFilterContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ProfanityFilterMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ProfanityFilterResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ProfanityFilterMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ProfanityFilterContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ProfanityFilterSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ProfanityFilterMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ProfanityFilterResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ProfanityFilterSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ProfanityFilterMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ProfanityFilterMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ProfanityFilterMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ProfanityFilterResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ProfanityFilterDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ProfanityFilterResult(['ok' => false])]);
        $this->expectException(ProfanityFilterError::class);
        ProfanityFilterDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ProfanityFilterResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ProfanityFilterMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ProfanityFilterMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ProfanityFilterBaseFeature();
        $b = new ProfanityFilterBaseFeature();
        ProfanityFilterFeatureAdd::call($ctx, $a);
        ProfanityFilterFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ProfanityFilterBaseFeature
    {
        $f = new ProfanityFilterBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ProfanityFilterFeatureAdd::call($ctx, self::named_feature('a'));
        ProfanityFilterFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        ProfanityFilterFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        ProfanityFilterFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        ProfanityFilterFeatureAdd::call($ctx, $replace);
        ...
        ProfanityFilterFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ProfanityFilterContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ProfanityFilterSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ProfanityFilterPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ProfanityFilterPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ProfanityFilterResponse(['headers' => null]),
            'result' => new ProfanityFilterResult([]),
        ]);
        ProfanityFilterResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ProfanityFilterResultBody::call($ctx);
