...
// ProfanityFilter SDK feature test
//
...
class FtCtrl extends ProfanityFilterControl
{
...
class FtClient
{
    ...
    private ProfanityFilterUtility $utility;

    public function __construct(ProfanityFilterUtility $utility, array $options)
    {
    ...
    public function get_utility(): ProfanityFilterUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ProfanityFilterContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ProfanityFilterContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ProfanityFilterContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ProfanityFilterConfig::make_feature($name);
    ...
    private static function buildUrl(ProfanityFilterSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ProfanityFilterError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ProfanityFilterResponse($fetched);
        }
    ...
    private function fail(ProfanityFilterContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ProfanityFilterContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
