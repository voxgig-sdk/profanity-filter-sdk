...
(0, node_test_1.describe)('PlainEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'plain.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set PROFANITY_FILTER_TEST_PLAIN_ENTID JSON to run live');
            return;
        }
        ...
        let plain_ref01_data = Object.values(setup.data.existing.plain)[0];
        // LOAD
        const plain_ref01_ent = client.Plain();
        const plain_ref01_match_dt0 = {};
        const plain_ref01_data_dt0 = (await plain_ref01_ent.load(plain_ref01_match_dt0)).data();
        (0, node_assert_1.default)(null != plain_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/plain/PlainTestData.json');
    ...
    let idmap = transform(['plain01', 'plain02', 'plain03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    const idmapEnvVal = process.env['PROFANITY_FILTER_TEST_PLAIN_ENTID'];
...
//# sourceMappingURL=PlainEntity.test.js.map