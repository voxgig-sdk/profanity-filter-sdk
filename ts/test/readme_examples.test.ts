...
//     (\`new ProfanityFilterSDK(...)\` / \`ProfanityFilterSDK.test(...)\` become
//     \`ProfanityFilterSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ProfanityFilterSDK'
...
const TEST_SEED = {"entity":{"containsprofanity":{"test01":{"id":"test01"}},"json":{"test01":{"id":"test01"}},"plain":{"test01":{"id":"test01"}},"xml":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ProfanityFilterSDK(...)\` and any
// \`ProfanityFilterSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ProfanityFilterSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ProfanityFilterSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
