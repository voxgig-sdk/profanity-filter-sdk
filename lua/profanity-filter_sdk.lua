...
require("profanity-filter_types")
...
local ProfanityFilterSDK = {}
ProfanityFilterSDK.__index = ProfanityFilterSDK
...
ProfanityFilterSDK._make_feature = _make_feature


function ProfanityFilterSDK.new(options)
  local self = setmetatable({}, ProfanityFilterSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ProfanityFilterSDK:options_map()
...
function ProfanityFilterSDK:get_utility()
  return Utility.copy(self._utility)
end


function ProfanityFilterSDK:get_root_ctx()
  return self._rootctx
end


function ProfanityFilterSDK:prepare(fetchargs)
...
function ProfanityFilterSDK:direct(fetchargs)
...
function ProfanityFilterSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function ProfanityFilterSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ProfanityFilterSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ProfanityFilterSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ProfanityFilterSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ProfanityFilterSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ProfanityFilterSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ProfanityFilterSDK.new(sdkopts)
  sdk.mode = "test"
