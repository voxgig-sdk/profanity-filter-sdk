...
local PlainEntity = {}
PlainEntity.__index = PlainEntity


function PlainEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PlainEntity)
  self._name = "plain"
  self._client = client
...
function PlainEntity:get_name()
  return self._name
end


function PlainEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PlainEntity.new(self._client, opts)
end
...
function PlainEntity:mark_deleted()
  self._deleted = true
end


function PlainEntity:deleted()
  return true == self._deleted
end


function PlainEntity:data_set(args)
  if args ~= nil then
...
function PlainEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PlainEntity:match_set(args)
  if args ~= nil then
...
function PlainEntity:match_get()
...
function PlainEntity:stream(action, args, callopts)
...
function PlainEntity:_run_op(ctx, post_done)
  local utility = self._utility
