...
local XmlEntity = {}
XmlEntity.__index = XmlEntity


function XmlEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, XmlEntity)
  self._name = "xml"
  self._client = client
...
function XmlEntity:get_name()
  return self._name
end


function XmlEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return XmlEntity.new(self._client, opts)
end
...
function XmlEntity:mark_deleted()
  self._deleted = true
end


function XmlEntity:deleted()
  return true == self._deleted
end


function XmlEntity:data_set(args)
  if args ~= nil then
...
function XmlEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function XmlEntity:match_set(args)
  if args ~= nil then
...
function XmlEntity:stream(action, args, callopts)
...
---@param reqmatch XmlLoadMatch
---@param ctrl? table
---@return Xml
---@return string? err
function XmlEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function XmlEntity:_run_op(ctx, post_done)
  local utility = self._utility
