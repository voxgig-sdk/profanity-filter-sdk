# ProfanityFilter Lua SDK

The Lua SDK for the ProfanityFilter API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Containsprofanity(nil):load({ id = "example_id" }, nil)
...
local result, err = client:ProfanityFilter(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── profanity-filter_sdk.lua    -- Main SDK module
...
The main module (`profanity-filter_sdk`) exports the SDK constructor
