...
boru-driven command-line client **and** interactive REPL for the ProfanityFilter
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/profanity-filter-cli)
make build

# 2. See usage (words, entities, env vars)
./profanity-filter-cli --help

# 3. Provide credentials once, via the environment
export PROFANITY_FILTER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./profanity-filter-cli load 1 containsprofanity            # {id:1} shorthand
./profanity-filter-cli load '{id:1}' containsprofanity       # explicit match map

# 5. Override the API base URL for a single call
PROFANITY_FILTER_BASE=https://api.example.com ./profanity-filter-cli load 1 containsprofanity

# 6. No arguments -> interactive REPL
./profanity-filter-cli
profanity-filter> load 1 containsprofanity
profanity-filter> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/profanity-filter-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/profanity-filter-cli load 1 containsprofanity
   ```
...
```sh
./profanity-filter-cli load 1 containsprofanity          # scalar shorthand for {id:1}
./profanity-filter-cli load '{id:1}' containsprofanity     # explicit match map
```
...
./profanity-filter-cli load 1 containsprofanity
```
...
Run with no arguments to open a REPL (prompt `profanity-filter>`). Each line is
evaluated as its own boru expression:

```text
$ ./profanity-filter-cli
profanity-filter> load 1 containsprofanity
profanity-filter> /help
profanity-filter> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `containsprofanity`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/profanity-filter-cli`. |
...
(`./profanity-filter-cli <expr>`) and interactively (the REPL), and expressions compose the
