// Command ProfanityFilter-cli is a CLI + REPL for the ProfanityFilter SDK,
...
const prompt = "profanity-filter"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "containsprofanity json plain xml"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PROFANITY_FILTER_APIKEY (api key), PROFANITY_FILTER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PROFANITY_FILTER_APIKEY carries the API key and
	// PROFANITY_FILTER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PROFANITY_FILTER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PROFANITY_FILTER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewProfanityFilterSDK(opts)
